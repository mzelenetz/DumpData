...
SELECT
		...
		, FLOOR(ip.DISCHARGED - ip.ADMITTED) * 1.0/drg.MCWEIGHT AS OPERATIONAL_CMI_ADJ_LOS
		, CASE WHEN FLOOR(ip.DISCHARGED - ip.ADMITTED) = 0 THEN 1 ELSE FLOOR(ip.DISCHARGED - ip.ADMITTED) END * 1.0/drg.MCWEIGHT AS FINANCE_CMI_ADJ_LOS
...
WHERE ip.DISCHARGED IS NOT NULL AND DISCHARGED >= TO_DATE('2021-05-01', 'YYYY-MM-DD') AND REGEXP_LIKE(fac.FACILITYDESC, '(white plains)|(wph)', 'i')-- (LOWER(fac.FACILITYDESC) LIKE '%white plains%' OR (LOWER(fac.FACILITYDESC) LIKE '%wph%'))
	AND EXTRACT(MONTH FROM DISCHARGED) = EXTRACT(MONTH FROM SYSDATE) AND EXTRACT(YEAR FROM DISCHARGED) = EXTRACT(YEAR FROM SYSDATE)
ORDER BY DISCHARGED
